...
export function AuthPage({ signup = false }: { signup?: boolean }) {
  ...
  async function submit(e: FormEvent) {
    ...
    if (signup && form.churchName.trim().length < 2)
      return setError("Enter your church name.");
    ...
    try {
      const r = await api.post(
        ...
        signup ? form : { email: form.email, password: form.password },
      );
    ...
    <main className="grid min-h-screen place-items-center p-5">
      <form onSubmit={submit} className="card w-full max-w-md">
        ...
        <button disabled={busy} className="btn primary mt-6 w-full">
          {busy ? "Creating account…" : signup ? "Register church" : "Sign in"}
        </button>
      </form>
    </main>
  );
