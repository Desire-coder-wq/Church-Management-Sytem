...
export function ManagementPage({
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      setSuccess(`${title.slice(0, -1)} saved successfully.`);
      setOpen(false);
      setForm({});
      load();
    } catch (e) {
    ...
    <>
      <div className="flex justify-between">
        <div>
          ...
          <p className="text-sm text-muted">Live records from Neon.</p>
        </div>
        {fields.length > 0 && (
      ...
      {open && (
        <form onSubmit={submit} className="card mt-6 grid gap-4 md:grid-cols-2">
          {fields.map((field) => (
          ...
          <tbody>
            {rows.map((x) => (
              <tr className="border-t border-line" key={x.id}>
                <td className="p-3">
                  {x.fullName ?? x.name ?? x.member?.fullName ?? "Collection"}
                </td>
                <td>
                  {x.phone ??
                    x.campaign?.name ??
                    (x.amount ? ugx(x.amount) : (x.message ?? "—"))}
                </td>
                <td>{x.status ?? "—"}</td>
              </tr>
            ))}
