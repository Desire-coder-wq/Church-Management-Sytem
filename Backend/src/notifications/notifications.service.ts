...
class TwilioSmsProvider implements SmsProvider {
  ...
  async send(phone: string, message: string) {
    ...
    const result = await this.client.messages.create({ body: message, from: fromNumber, to: phone });
...
export class NotificationsService {
  ...
  private readonly smsRateLimit = new Map<string, { count: number; resetAt: number }>();
  ...
  constructor(private readonly db: PrismaService, private readonly config: ConfigService) {
    ...
    this.provider = provider === 'twilio' ? new TwilioSmsProvider(config) : new MockSmsProvider();
  }
  ...
  async sendSms(memberId: string, phone: string, message: string, type: SmsType): Promise<void> {
    if (!this.checkSmsRateLimit(phone)) {
      await this.db.smsNotification.create({
        data: { memberId, phone, message, type, status: 'FAILED', failureReason: 'Rate limit exceeded (max 10 SMS/hour)' },
      });
    ...
    try {
      ...
      await this.db.smsNotification.update({
        where: { id: notification.id },
        data: { status: 'FAILED', failureReason: error instanceof Error ? error.message : 'Unknown error', retryCount: { increment: 1 } },
      });
      this.logger.error(`SMS failed to ${phone}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      throw error;
    }
  }

  async sendPledgeCreatedNotification(memberId: string, phone: string, campaignName: string, amount: number, dueDate: Date) {
  ...
  async sendPaymentReceivedNotification(memberId: string, phone: string, campaignName: string, amount: number, balance: number) {
  ...
  async sendPaymentReminderNotification(memberId: string, phone: string, campaignName: string, balance: number, dueDate: Date) {
  ...
  async sendFullyPaidNotification(memberId: string, phone: string, campaignName: string) {
  ...
  async retryFailedNotifications() {
    ...
    for (const notification of failed) {
      try {
        ...
        await this.db.smsNotification.update({
          where: { id: notification.id },
          data: { failureReason: error instanceof Error ? error.message : 'Retry failed', retryCount: { increment: 1 } },
        });
      }
    }
  }
