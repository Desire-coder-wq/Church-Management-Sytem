...
export class MembersController {
  ...
  @Roles("ADMIN", "STAFF")
  @Get(":id")
  async detail(@Req() req: { user: Session }, @Param("id") id: string) {
    const member = await this.db.member.findFirst({
      where: { id, churchId: requireChurch(req.user) },
      include: {
        group: true,
        pledges: { include: { campaign: true, collections: true } },
      },
    });
    if (!member)
      throw new NotFoundException("Member not found in your church.");
    return member;
  }
  @Roles("ADMIN")
  @Post()
  create(@Req() req: { user: Session }, @Body() dto: MemberDto) {
    ...
    return this.db.member.create({
      data: {
        ...
        group: {
          connectOrCreate: {
            where: { churchId_name: { churchId, name: dto.group } },
            create: { churchId, name: dto.group },
          },
        },
      },
      include: { group: true },
