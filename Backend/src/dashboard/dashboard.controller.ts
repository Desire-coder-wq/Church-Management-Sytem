...
export class DashboardController {
  constructor(
    ...
    private readonly pledges: PledgesService,
  ) {}
  @Roles('ADMIN', 'STAFF')
  @Get() async summary(@Req() req: { user: Session }) {
    ...
    const [rows, campaigns, members] = await Promise.all([
      this.pledges.list(churchId),
      this.db.campaign.findMany({ where: { churchId } }),
      this.db.member.count({ where: { churchId } }),
    ]);
    ...
    return {
      ...
      remainingToTarget: Math.max(0, target - paid),
      recentCollections: collections
        .sort((a, b) => b.paymentDate.getTime() - a.paymentDate.getTime())
        .slice(0, 5),
      upcoming: rows
        .filter((row) => row.balance > 0 && row.dueDate >= new Date())
        .slice(0, 5),
